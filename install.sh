...
_select_kcp_mode() {
    ...
    mc="${mc:-1}"
    case "$mc" in
        0) echo "normal" ;; 1) echo "fast"  ;; 2) echo "fast2" ;;
        3) echo "fast3"  ;; 4) echo "manual" ;; *) echo "fast"  ;;
    esac
}
...
configure_kcp_server() {
    ...
    # [4] KCP Mode
    local mode
    mode=$(_select_kcp_mode)
    local extra_params=""
    [ "$mode" = "manual" ] && extra_params=$(_manual_kcp_params)
    ...
    mkdir -p "$CONFIG_DIR/kcp"
    {
        ...
        echo "    block: \"${enc}\""
        echo "    mtu: ${mtu}"
        if [ "$mode" = "manual" ] && [ -n "$extra_params" ]; then
...
configure_server() {
    while true; do
        local ch
        ch=$(_proto_menu "Configure Server  (Abroad / Kharej)")
        case "$ch" in
...
configure_client() {
    while true; do
        local ch
        ch=$(_proto_menu "Configure Client  (Iran / Domestic)")
        case "$ch" in
